...
- (NSURL *)createRealmWithName:(SEL)sel permissions:(void (^)(RLMRealm *realm))block {
    ...
    p = [[RLMSyncPermission alloc] initWithRealmPath:[url path] identity:self.userC.identity
                                         accessLevel:RLMSyncAccessLevelRead];
    APPLY_PERMISSION(p, admin);
    [self waitForSync:adminRealm user:admin];
...
static void createPermissions(RLMArray *permissions) {
    // Removing the "everyone" permission doesn't work as it'll just get recreated
    for (RLMPermission *p in permissions) {
        p.canCreate = false;
        p.canRead = false;
        p.canQuery = false;
        p.canDelete = false;
    ...
    [permissions addObject:@{@"role": getRole(permissions.realm, @"reader"), @"canRead": @YES, @"canQuery": @YES}];
    [permissions addObject:@{@"role": getRole(permissions.realm, @"writer"), @"canUpdate": @YES}];
...
- (void)testRealmReadAccess {
    ...
    XCTAssertFalse([userARealm privilegesForRealm].update);
...
- (void)testRealmWriteAccess {
    ...
    XCTAssertFalse([userARealm privilegesForRealm].setPermissions);
    ...
    [self subscribeToRealm:userBRealm type:[SyncObject class] where:@"TRUEPREDICATE"];
    XCTAssertFalse([userARealm privilegesForRealm].setPermissions);
    XCTAssertTrue([userBRealm privilegesForRealm].read);
...
- (void)testRealmManagePermissions {
    ...
    NSLog(@"%@", [RLMRealmPermission objectInRealm:userCRealm].permissions);
...
- (void)testClassRead {
    ...
    [self subscribeToRealm:userBRealm type:[SyncObject class] where:@"TRUEPREDICATE"];
    XCTAssertTrue([userBRealm privilegesForRealm].read);
    XCTAssertFalse([userBRealm privilegesForClass:SyncObject.class].read);
    CHECK_COUNT(0, SyncObject, userBRealm);
}
